...
const UMOUNT_CONFIG_MAGIC: u32 = 0x4B53554D; // KSUM

pub fn save_umount_config() -> Result<()> {
    let list_output = ksucalls::umount_list_list()
        .context("Failed to get umount list from kernel")?;
    ...
    let mut file = fs::File::create(config_path)
        .context("Failed to create umount config file")?;
...
pub fn load_umount_config() -> Result<()> {
    ...
    let file = fs::File::open(config_path)
        .context("Failed to open umount config file")?;
    ...
    ksucalls::umount_list_wipe()
        .context("Failed to wipe existing umount list")?;
...
pub fn clear_umount_config() -> Result<()> {
    ...
    ksucalls::umount_list_wipe()
        .context("Failed to wipe kernel umount list")?;
    
    // Delete config file if it exists
    if config_path.exists() {
        fs::remove_file(config_path)
            .context("Failed to remove umount config file")?;
